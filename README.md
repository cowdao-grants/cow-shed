...
Owners can run hooks without a signature through `COWShed.trustedExecuteHooks` on an existing shed. `COWShedFactory.executeOwnHooks` deploys the caller's shed if needed and runs the hooks in the same transaction.
